...
```{r precip, fig.width = 9, fig.height = 6, message = F}
data_temp_8 = data %>%
...
```{r corr-2, fig.width = 9, fig.height = 6, message = F}
...
### **4.2. The links between Temperature and Humidity**

Temperature and Humidity are different but they are related to each other.The relation between humidity and temperature formula simply says they are inversely proportional. If temperature increases it will lead to a decrease in relative humidity, thus the air will become drier whereas when temperature decreases, the air will become wet means the relative humidity will increase.We want to show this information with their correlation matrix. According to the records of the data they have a negative correlation as -0.54.

```{r corr_temp_humid, fig.width = 9, fig.height = 6, message = F}
library(PerformanceAnalytics)
chart.Correlation(data[c(5,7)], histogram = TRUE, method = "pearson")
```
